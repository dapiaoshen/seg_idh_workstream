...
from models.TransBTS.fusion import BilinearFusion,TrilinearFusion_A,TrilinearFusion_B
...
class Genomic_Clinical(nn.Module):
    def __init__(self):
        ...
        encoder1 = nn.Sequential(nn.Linear(9, 32), nn.ReLU(), nn.Dropout(p=0.2))  # 9, 32 // 9, 16
        encoder2 = nn.Sequential(nn.Linear(32, 16), nn.ReLU(), nn.Dropout(p=0.2))  # 32, 16 // 16, 32
        encoder3 = nn.Sequential(nn.Linear(16, 9), nn.ReLU(), nn.Dropout(p=0.2))  # 32, 9 // 32, 64
...
class Cox_network(nn.Module):
    def __init__(self):
        ...
        self.fusion =BilinearFusion(skip=1, use_bilinear=1, gate1=1, gate2=1, dim1=128, dim2=512, scale_dim1=1, scale_dim2=1, mmhid=256, dropout_rate=0.2)
        # self.fusion = TrilinearFusion_A(skip=1, use_bilinear=1, gate1=1, gate2=1, gate3=1, dim1=128, dim2=512, dim3=64, scale_dim1=2,
        #         scale_dim2=4, scale_dim3=1, mmhid=256, dropout_rate=0.2)
        encoder1 = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Dropout(p=0.2))
        encoder2 = nn.Sequential(nn.Linear(128, 32), nn.ReLU(), nn.Dropout(p=0.2))
    ...
    def forward(self,x4_1,encoder_output): #genomic
        deep_1,deep_2 = self.feature_fusion_layer(x4_1, encoder_output)

        # genomic = self.genomic_module(genomic)

        fusion_feature = self.fusion(deep_1,deep_2)
        out = self.encoder(fusion_feature)
        # genomic = self.encoder_c(genomic)
        # out = self.classifier(torch.cat([out,genomic],dim=-1))
...
if __name__ == '__main__':
    ...
    input_tensor2 = torch.rand(2, 2)
    print(input_tensor)
    print(input_tensor2)
    print(input_tensor*input_tensor2)
    # x = input_tensor.permute(0, 2, 3, 4, 1).contiguous()
    # x = x.view(x.size(0), -1, 512)
    # print(x.shape)
    # model = TransBTS_1(dataset='brats', _conv_repr=True, _pe_type="learned")
    # out = model.forward(input_tensor)
    # summary(model.to(torch.device(device)), (4, 128, 128, 128), batch_size=0, device=device)
    # with torch.no_grad():
    #     import os
    #     # os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    #     # cuda0 = torch.device('cuda:0')
    #     x = torch.rand((1, 4, 128, 128, 128), device='cpu')
    #     model = TransBTS_1(dataset='brats', _conv_repr=True, _pe_type="learned")
    #     # model.cuda()
    #     y = model(x)
    #     print(y)
    ...
    # device = 'cpu'
    # input_tensor = torch.rand(1, 8, 128, 128, 128)
    # model = ChannelFusion(8)
    # out = model.forward(input_tensor)
    # summary(model.to(torch.device(device)), (8, 128, 128, 128), batch_size=0, device=device)
