...
class FeatureFusionModule(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Conv3d(out_channels, out_channels//4, kernel_size=1)
        self.relu = nn.LeakyReLU(inplace=True)
        self.conv2 = nn.Conv3d(out_channels//4, out_channels, kernel_size=1)
        self.sigmoid = nn.Sigmoid()
        self.avgpool = nn.AdaptiveAvgPool3d(output_size=(1, 1, 1))
...
class IDH_network(nn.Module):
    def __init__(self):
        ...
        self.avg_pool_3d = nn.AvgPool3d(16, 1)
        self.max_pool_3d = nn.MaxPool3d(16, 1)
        self.Hidder_layer_1 = nn.Linear(1280, 512)  #1280  #1088
        self.Hidder_layer_2 = nn.Linear(512, 32) #512
        self.drop_layer = nn.Dropout(p=0.2)
        self.classifier = nn.Linear(32, 3)
        #self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self,x4_1,encoder_output):  #x4_1,encoder_output
        ...
        y = self.classifier(x)
        return y
    # def forward(self,encoder_output):  #x4_1,encoder_output
    #     encoder_avg = self.avg_pool_3d(encoder_output)
    #     encoder_avg_max = self.max_pool_3d(encoder_output)
    #     encoder_avg = encoder_avg.view(encoder_avg.size(0), -1)
    #     encoder_avg_max = encoder_avg_max.view(encoder_avg_max.size(0), -1)
    #     x = torch.cat([encoder_avg,encoder_avg_max], dim=1)
    #     x = self.drop_layer(x)
    #     x = self.Hidder_layer_1(x)
    #     x = self.Hidder_layer_2(x)
    #     y = self.classifier(x)
...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        self.Unet1 = Unet(in_channels=2, base_channels=8, num_classes=4)
        self.Unet2 = Unet(in_channels=2, base_channels=8, num_classes=4)
        self.bn = nn.BatchNorm3d(128)
        self.relu = nn.LeakyReLU(inplace=True)
        self.ffm_1 = FeatureFusionModule(16, 16)
        self.ffm_2 = FeatureFusionModule(32, 32)
        self.ffm_3 = FeatureFusionModule(64, 64)
        self.ffm_4 = FeatureFusionModule(128,128)

    def encode(self, x):
        if self.conv_patch_representation:
            # combine embedding with conv patch distribution
            flair_t2 = torch.cat([x[:, 0:1],x[:, 3:4]],dim=1)
            t1ce_t1 = torch.cat([x[:, 1:2],x[:, 2:3]],dim=1)

            flair_t2_x1, flair_t2_x2, flair_t2_x3, flair_t2_x4 = self.Unet1(flair_t2)
            t1ce_t1_x1, t1ce_t1_x2, t1ce_t1_x3, t1ce_t1_x4 = self.Unet2(t1ce_t1)

            # flair_x1, flair_x2, flair_x3, flair_x4 = self.Unet(x[:, 0:1])
            # t1ce_x1, t1ce_x2, t1ce_x3, t1ce_x4 = self.Unet(x[:, 1:2])
            # t1_x1, t1_x2, t1_x3, t1_x4 = self.Unet(x[:, 2:3])
            # t2_x1, t2_x2, t2_x3, t2_x4 = self.Unet(x[:, 3:4])

            x1_1 = self.ffm_1(flair_t2_x1, t1ce_t1_x1)
            x2_1 = self.ffm_2(flair_t2_x2, t1ce_t1_x2)
            x3_1 = self.ffm_3(flair_t2_x3, t1ce_t1_x3)
            x4_1 = self.ffm_4(flair_t2_x4, t1ce_t1_x4)
