...
class BTS(TransformerBTS):
    def __init__(
        ...
        # self.boundary_out = BoundarySeg(4,4,4)

        self.bottem_boundary = BoundarySeg(128,16,4)
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        x8 = self.Enblock8_2(x8)   #(1, 128, 16, 16, 16)

        boundary = self.bottem_boundary(x8)
...
class BoundarySeg(nn.Module):
    def __init__(self, in_chan, mid_chan, n_classes, *args, **kwargs):
        super(BoundarySeg, self).__init__()
        self.conv = nn.Conv3d(in_chan,mid_chan,kernel_size=3,stride=1,padding=1,bias=False)
        self.bn = nn.BatchNorm3d(mid_chan)
        self.relu = nn.LeakyReLU()
        self.conv_out = nn.Conv3d(mid_chan, n_classes, kernel_size=1,bias=False)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        _, model = TransBTS(dataset='brats', _conv_repr=True, _pe_type=" ")
        #model.cuda()
        model.cpu()
        y = model(x)
